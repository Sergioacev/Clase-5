...
def extractFormRegularExpresion(regex, data):
    if data: 
...
resultado= extractFormRegularExpresion(regex, data)

for i in range(len(resultado)):
    print(f"La IP: {resultado[i][0]} y la fecha es: {resultado[i][1]} y la hora{resultado[i][2]}")